       ...
       validateChain() {
        this.app.get("/validatechain", async (req, res) => {
            try {
                ...
                if (arr) { return res.status(200).json(arr);} 
                else {return res.status(404).send('Chain cannot be validated');}
            } catch (error) {return res.status(500).send('Error has occured');}
        });
    }
