...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            var msg = `${address}:${new Date().getTime().toString().slice(0,-3)}`;
            console.log('requestMessageOwnership: ' + msg);   
            resolve(msg);
        });
    }
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (currentTime - messageTime >= 300) {
                    let error = new Error('Sorry, you are late, more than 5 minutes have passed');
                    console.error(error);
                    reject(error);
                    return;
                }
